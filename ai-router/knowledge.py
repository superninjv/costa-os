...
TOPIC_PATTERNS = {
    ...
    "costa-setup": r"(costa|theme|waybar|ags|shell\s*bar|ghostty|rofi|dunst|wallpaper|config|dotfile|chezmoi|setup|customize)",
    "dev-tools": r"(python|pyenv|node|nvm|rust|cargo|java|sdk|docker|compose|git|lazygit|zellij|kubectl|k9s|tree.sitter|ast|costa.ast|symbol|complexity|parse\s*file|firecrawl|web\s*scrap|scrape|crawl\s*site)",
    ...
    "customization": r"(custom|theme|color|wallpaper|font|opacity|animation|window\s*rule|config.*change|waybar|ags|shell\s*bar|bar\s*template|monitor.*bar|performance\s*bar|taskbar)",
    ...
    "security": r"(face\s*(auth|unlock|recognition|login)|howdy|ir\s*camera|touchscreen|touch\s*(gesture|input|screen)|squeekboard|hyprgrass|pam|biometric|firewall|ufw|security\s*scan|hardening|sysctl|encryption|bluetooth\s*secur)",
