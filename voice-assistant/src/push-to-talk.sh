...
OLLAMA_MODEL=$(cat "$_R/costa/ollama-smart-model" 2>/dev/null || echo "${COSTA_OLLAMA_DEFAULT:-qwen2.5:14b}")
OLLAMA_SUMMARY="$OLLAMA_FAST"

LOCKFILE="$_R/ptt.lock"
STATUS_FILE="$_R/ptt-status"
...
VAD_BUSY=$(cat $_R/vad-status 2>/dev/null)
...
VAD_STATUS=$(cat $_R/vad-status 2>/dev/null)
if [ "$VAD_STATUS" = "ready" ]; then
    ...
    pw-cat --record --format=s16 --rate=16000 --channels=1 $_R/ptt-raw.wav &
    REC_PID=$!
...
# Check audio
if [ ! -f $_R/ptt-raw.wav ] || [ "$(stat -c%s $_R/ptt-raw.wav 2>/dev/null)" -lt 2000 ]; then
    update_status "ready"
...
RESULT=$("$WHISPER_BIN" -m "$MODEL" -f $_R/ptt-raw.wav --no-timestamps -nt 2>/dev/null \
    | sed '/^$/d; s/^ *//; s/ *$//; s/\[.*\]//g' | tr -s ' ')
rm -f $_R/ptt-raw.wav
...
if [ "$CLAUDE_MODE" = "claude" ]; then
    echo "running" > $_R/ptt-voice-status
    echo "$RESULT" > $_R/ptt-voice-command
    echo "" > $_R/ptt-voice-output
    ...
    (
        ...
        if [ -n "$ROUTER_OUTPUT" ]; then
            echo "$ROUTER_OUTPUT" | jq -r '.response' > $_R/ptt-voice-output 2>&1
            echo "$ROUTER_OUTPUT" | jq -r '.route' > $_R/ptt-voice-model 2>&1
        fi

        # Handle timeout
        if [ $EXIT_CODE -eq 124 ]; then
            echo "timed out" > $_R/ptt-voice-status
            notify-send -t 3000 "Voice command timed out" -a "VoiceMode"
            sleep 10
            echo "idle" > $_R/ptt-voice-status
            rm -f $_R/ptt-voice-output $_R/ptt-voice-lastline $_R/ptt-voice-command $_R/ptt-voice-model
            exit 0
        fi

        # Handle errors -> escalate to interactive
        if [ $EXIT_CODE -ne 0 ] && [ $EXIT_CODE -ne 124 ]; then
            echo "interactive" > $_R/ptt-voice-status
            ...
            "$TERMINAL" --class=voice-claude -e bash -c "
                if [ -s $_R/ptt-voice-output ]; then
                    cat $_R/ptt-voice-output
                    echo ''
                fi
                echo '───────────────────────────────────────'
                if command -v claude &>/dev/null; then
                    EMODEL=\$(cat $_R/ptt-voice-model 2>/dev/null || echo sonnet)
            ...
            OUTPUT_LEN=$(wc -c < $_R/ptt-voice-output 2>/dev/null || echo "0")
            RAW_TEXT=$(sed 's/[*#`]//g' $_R/ptt-voice-output | tr '\n' ' ' | tr -s ' ' | head -c 200)
            ...
            echo "scroll" > $_R/ptt-voice-status
            echo "$SUMMARY" > $_R/ptt-voice-scroll
            notify-send -t 5000 "$SUMMARY" -a "VoiceMode"
            sleep 20
            echo "idle" > $_R/ptt-voice-status
            rm -f $_R/ptt-voice-output $_R/ptt-voice-lastline $_R/ptt-voice-command $_R/ptt-voice-scroll $_R/ptt-voice-model
        fi
    ) &disown
else
    # Type mode
